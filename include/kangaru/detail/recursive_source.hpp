...
namespace kangaru::sources {
	...
	struct with_recursive_construct {
		constexpr explicit with_recursive_construct(Source source) noexcept : source{std::move(source)} {}
		...
		template<detail::concepts::prvalue T, typename S> requires (not source_of<S, T>)
		struct provide_recursive {
			auto operator()(detail::concepts::forwarded<with_recursive_construct> auto&& source) -> decltype(spread_injector<decltype(kangaru::ref(source))>{kangaru::ref(source)}(recurse_construct<T>{})) {
		...
		template<typename T, detail::concepts::forwarded<with_recursive_construct> Self>
		...
		template<detail::concepts::prvalue T, detail::concepts::forwarded<with_recursive_construct> Self, typename S = detail::utility::forward_like_t<Self, Source>>
